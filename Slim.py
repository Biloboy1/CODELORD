...
# Time Succes decompile : 2022-04-19 22:40:40.529027
...
{H}[{P}>{H}]{P} Author: Slimwiz
{H}[{P}>{H}]{P} Whatsapp: +2349138017570
{H}[{P}>{H}]{P} Facebook : facebook.com/Anonymous
{K}[{P}•{K}]{P}—————————————————————————————""")
...
def menu():
	...
	print(balmond+u+" =>"+h+" Slim_Op/OK_%s.txt"%(hck))
	print(balmond+u+" =>"+k+" Slim_Cp/CP_%s.txt"%(hck))
	print(u+line)
	print(s+"\n{"+m+"1"+s+"}"+B+" Crack Facebook from Publik "+H+"{5000}")
	print(s+"{"+m+"2"+s+"}"+B+" Crack From Followers Publik "+H+"{5000}")
	...
	print(s+"{"+m+"8"+s+"}"+H+" Show Result Crack")
...
def result():
	...
	if pilih=="1" or pilih=="01":
		...
		if len(lisaa)==0:
			print("\n"+balmond+k+" Slim CP")
			print(balmond+m+" Tidak Ada Slim Cp")
			...
			try:
				binatang = open("Slim_Cp/%s"%(marjan))
			except IOError:
				jalan(balmond+l+" Nama File Tidak Ada")
				time.sleep(0.5)
				menu()
		print(""+l)
		bilur = os.system("cd Slim_Cp && cat %s"%(marjan))
		input("\n"+balmond+l+" Kembali")
		time.sleep(0.5)
		menu()
	elif pilih=="2" or pilih=="02":
		try:
			lisaa = os.listdir("Slim_Ok")
		except FileNotFoundError:
			jalan(balmond+m+" Direktori Tidak Ditemukan")
			time.sleep(0.5)
			menu()
		if len(lisaa)==0:
			...
			marjan = input(balmond+l+" File : "+h+"")
			try:
				binatang = open("Slim_Ok/%s"%(marjan))
			except IOError:
				jalan(balmond+l+" Nama File Tidak Ada")
				time.sleep(0.5)
				menu()
		print(""+l)
		bilur = os.system("cd Slim_Ok && cat %s"%(marjan))
		input("\n"+balmond+l+" Kembali")
		time.sleep(0.5)
		menu()
	elif pilih=="0" or pilih=="00":
